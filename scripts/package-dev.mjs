...
try {
  ...
  pkg.name = "oh-my-pi-chat-dev";
  pkg.displayName = "OMP Chat (Dev Preview)";
  pkg.publisher = "localdev";

  let pStr = JSON.stringify(pkg, null, 2);
  pStr = pStr.replace(/ompChat\.sidebar/g, "ompChatDev.sidebar");
  pStr = pStr.replace(/"ompChat": \[/g, '"ompChatDev": [');
  pStr = pStr.replace(/"id": "ompChat"/g, '"id": "ompChatDev"');
  ...
  cvp = cvp.replace(/ompChat\.sidebar/g, "ompChatDev.sidebar");
  writeFileSync("src/chat/chatViewProvider.ts", cvp);

  let ext = readFileSync("src/extension.ts", "utf8");
  ext = ext.replace(/ompChat\.sidebar/g, "ompChatDev.sidebar");
  writeFileSync("src/extension.ts", ext);

  // 4. Build and package
  console.log("Building development preview package...");
  execSync("npm run build", { stdio: "inherit" });
  execSync("npx vsce package --no-dependencies --allow-missing-repository -o oh-my-pi-chat-dev.vsix", { stdio: "inherit" });
  console.log("Successfully created oh-my-pi-chat-dev.vsix!");
} finally {
